...
**任务**：
- [ ] 删除 `domain/agents/blood_pressure/agent.py`
- [ ] 删除 `domain/agents/appointment/agent.py`
- [ ] 更新相关导入（如果有）
- [ ] 验证功能不受影响
...
- [ ] 基于AgentRegistry动态添加Agent节点
- [ ] 动态生成路由决策逻辑
- [ ] 动态添加回边
- [ ] 保持"Agent在路由图创建时一次性创建"的原则
...
- 单元测试：测试路由图的动态构建
- 集成测试：测试完整的路由流程
- 扩展性测试：添加新Agent验证动态构建

#### 步骤2.4：增强Agent配置（0.5天）

**任务**：
- [ ] 更新 `config/agents.yaml` 配置格式
- [ ] 添加路由相关配置（node_name, intent_type等）
- [ ] 添加占位符配置
- [ ] 添加Langfuse模版配置
...
4. **更新配置**
   - 在 `config/agents.yaml` 中添加 `langfuse_template` 配置
   - 添加 `placeholders` 配置（如果需要）

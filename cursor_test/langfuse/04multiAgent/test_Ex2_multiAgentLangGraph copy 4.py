...
def main():
    ...
    # 如果不指定（trace_id=None），将使用默认的 handler（自动生成 trace_id）
    execute_graph(graph, langfuse_handler, question2, question_id="2", trace_id=None)
    
    # 示例：使用自定义 trace_id
    import secrets
    custom_trace_id = secrets.token_hex(16)  # 生成 32 位十六进制字符
    execute_graph(graph, langfuse_handler, question1, question_id="1", trace_id=custom_trace_id)

    ...
    try:
        ...
        if normalized_trace_id:
            try:
                ...
                langfuse_client.update_current_trace(id=normalized_trace_id, **update_params)
            except (TypeError, AttributeError):
                # 如果 SDK 不支持 id 参数，记录警告但继续执行
                logger.warning(
                    f"Langfuse SDK 可能不支持 id 参数，将使用默认行为。trace_id={normalized_trace_id}"
                )
                langfuse_client.update_current_trace(**update_params)
        else:
            # 由 Langfuse 生成 trace_id
            langfuse_client.update_current_trace(**update_params)
        
        # 将Trace ID存储到上下文变量中
        actual_trace_id = normalized_trace_id or trace_id
        if actual_trace_id:

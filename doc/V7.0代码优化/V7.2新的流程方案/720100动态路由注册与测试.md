...
本文档描述V7.2版本动态流程系统的最小可行版本（MVP）开发计划。本版本旨在验证核心架构设计，实现最基本的流程配置化和动态加载能力。
...
- [ ] 实现流程定义类（domain/flows/definition.py）
- [ ] 实现YAML解析器（domain/flows/parser.py）
- [ ] 实现图构建器（domain/flows/builder.py）
- [ ] 实现流程管理器（domain/flows/manager.py）
- [ ] 实现状态定义（domain/state.py）

### 7.3 第三阶段：提示词管理

- [ ] 实现提示词加载器（infrastructure/prompts/loader.py）
- [ ] 实现提示词管理器（infrastructure/prompts/manager.py）
- [ ] 创建医疗分身Agent流程提示词文件（config/flows/medical_agent/prompts/）
- [ ] 创建工作计划Agent流程提示词文件（config/flows/work_plan_agent/prompts/）

### 7.4 第四阶段：Agent系统

- [ ] 实现Agent工厂（domain/agents/factory.py）
- [ ] 实现Agent注册表（domain/agents/registry.py）
- [ ] 实现节点工厂（根据节点类型创建节点函数）

### 7.5 第五阶段：工具系统

- [ ] 实现工具注册表（domain/tools/registry.py）
- [ ] 实现血压记录工具（简化版）
- [ ] 实现工具包装器（TokenContext注入）

### 7.6 第六阶段：流程配置

- [ ] 创建模型供应商配置文件（config/model_providers.yaml）
- [ ] 创建医疗分身Agent流程目录和配置（config/flows/medical_agent/flow.yaml）
- [ ] 创建工作计划Agent流程目录和配置（config/flows/work_plan_agent/flow.yaml）
- [ ] 创建流程加载配置（config/flow_loader.yaml）
- [ ] 创建医疗分身Agent流程提示词文件（config/flows/medical_agent/prompts/）
- [ ] 创建工作计划Agent流程提示词文件（config/flows/work_plan_agent/prompts/）

### 7.7 第七阶段：API层

- [ ] 实现FastAPI应用（backend/main.py）
- [ ] 实现API路由（backend/app/api/routes.py）
- [ ] 实现请求/响应模型

### 7.8 第八阶段：前端界面

- [ ] 创建简单的聊天界面（frontend/index.html）
- [ ] 实现前端与后端通信

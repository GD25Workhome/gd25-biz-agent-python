...
**创建时间**：2025-01-XX  
**最后更新**：2025-01-XX  
**对应流程**：`config/flows/medical_agent_v5`

**版本变更说明**：
- V2.0：简化节点职责，节点1专注问题加工，节点2采用定制化实现，节点3承担逻辑二次判定和工具调用
- V1.0：初始版本，节点1进行意图识别
...
节点2：RAG检索节点（定制化实现）
    ↓
节点3：核心Agent（统一处理所有业务流程）
```

**设计目标**：
1. **节点1（问题加工节点）**：分析患者最新发言，结合历史聊天上下文，提取问题的核心要点，优化为适合向量库检索的查询文本
2. **节点2（RAG检索节点）**：基于节点1的优化查询文本，从向量库检索相关示例和资料；本阶段采用定制化实现，配置简化
3. **节点3（核心Agent节点）**：根据前置RAG检索的信息，进行逻辑二次判定，处理所有业务流程（记录、查询、QA、greeting等），并执行工具调用

**核心优势**：
- 职责清晰：节点1专注问题加工，节点2专注信息检索，节点3专注业务处理
- 提示词精简：节点3只保留核心流程逻辑，问答示例通过RAG动态获取
- 知识库可扩展：优秀的历史问答和科普资料可随时更新到向量库
- 灵活实现：节点2采用定制化实现，便于快速迭代和优化
...
**注意**：节点1不再进行意图识别和路由判断，这些职责由节点3承担。
...
5. **结合上下文优化**：如果当前问题依赖历史上下文，将上下文信息融入查询文本
...
- `query_text`（必填）：优化后的查询文本，用于向量库检索
  - 必须遵循查询文本优化原则
  - 应该去除口语化表达，使用标准术语
  - 结合历史上下文，确保查询文本完整表达患者的问题
  - 长度建议控制在 50 字以内
  - 如果无法提取有效问题（如纯问候），可以设置为空字符串 `""`

- `keywords`（必填）：提取的关键实体列表（数组）
  - 疾病名称：["高血压", "糖尿病"]
  - 症状：["头晕", "胸闷"]
  - 药物：["降压药", "阿司匹林"]
  - 数据类型：["血压", "收缩压", "舒张压"]
...
提示词文件位置：`config/flows/medical_agent_v5/prompts/10-before-rag.md`
...
节点2负责：
1. **向量库检索**：基于节点1的 `query_text` 和 `keywords`，从向量库检索相关历史问答和科普资料
2. **用户信息查询**：查询用户基础信息（作为节点3提示词的占位符）
3. **结果整合**：将检索结果和用户信息整合，传递给节点3

**实现方式**：节点2是一个**函数节点（function node）**，采用定制化实现，配置简化。

### 3.2 配置说明

在 `flow.yaml` 中，节点2的配置已简化，只保留类型和名称：

```yaml
- name: retrieval_node
  type: function  # 函数节点，非Agent
  # config:  # 本阶段采用定制化实现，配置简化
```

**设计考虑**：
- 本阶段采用定制化实现，便于快速迭代和优化
- 具体的检索策略、相似度阈值、Top-K等参数在代码中实现
- 未来可根据需要扩展配置项
...
节点3是统一的Agent，负责处理所有业务流程：

1. **逻辑二次判定**：
   - 根据节点1的问题加工结果和节点2的RAG检索信息，进行意图识别和逻辑判断
   - 判断用户意图类型（record、query、qa、greeting）
   - 进行业务逻辑的二次确认和验证
...
3. **工具调用**：
   - 根据业务逻辑判断，调用相应的工具（记录工具、查询工具等）
   - 处理工具调用的结果，生成用户友好的回复

**核心特点**：
- 节点3承担了意图识别和路由的职责（节点1不再做意图识别）
- 节点3根据RAG检索的信息进行逻辑二次判定，确保判断准确性
- 节点3统一处理所有业务流程，避免多Agent分离带来的复杂性
...
**输入信息**：
- 节点1的输出：`query_text`（优化后的问题）、`keywords`（关键词列表）
- 节点2的输出：`retrieved_examples`（检索到的示例）、`user_info`（用户信息）
- 历史对话上下文

**判定内容**：
1. **意图识别**：判断用户意图类型（record、query、qa、greeting）
2. **业务逻辑验证**：验证用户需求的完整性和合理性
3. **数据完整性检查**：对于记录类操作，检查必要数据是否完整
4. **安全边界判断**：对于问答类操作，判断是否涉及安全边界场景

**实现方式**：
- 在提示词中明确说明判定规则和流程
- Agent根据RAG检索的示例和用户信息，结合业务规则进行判断
- 如果判断不明确，可以要求用户澄清

#### 4.3.2 工具调用策略

节点3需要支持所有工具，并根据判定结果调用相应工具：

**工具列表**：
- **记录工具**：`record_blood_pressure`、`update_blood_pressure`、`record_medication`、`record_symptom`、`record_health_event`
- **查询工具**：`query_blood_pressure`、`query_medication`、`query_symptom`、`query_health_event`

**工具调用原则**：
1. **根据意图调用**：根据二次判定结果，调用对应的工具
2. **数据完整性检查**：调用记录工具前，确保必要数据完整
3. **结果处理**：处理工具调用的结果，生成用户友好的回复
4. **错误处理**：处理工具调用失败的情况，提供友好的错误提示
...
nodes:
  # 节点1：问题加工节点
  - name: intent_recognition  # 注意：节点名称保持为 intent_recognition，但职责已改为问题加工
    type: agent
    config:
...
class RouterState(TypedDict):
    ...
    current_intent: Optional[str]  # 节点3进行二次判定后设置
    current_agent: Optional[str]
    
    # 节点1输出（问题加工结果）
    query_text: Optional[str]  # 优化后的查询文本
    keywords: Optional[List[str]]  # 提取的关键词列表
...
初始状态
  ↓
节点1（问题加工）：更新 query_text、keywords
  ↓
节点2（RAG检索）：更新 retrieved_examples、user_info
  ↓
节点3（核心Agent）：
  - 根据 query_text、keywords、retrieved_examples 进行逻辑二次判定
  - 更新 current_intent
  - 处理业务流程，调用工具
  - 生成回复
  ↓
...
# 执行检索
results = vector_db_search(query_text, keywords)
```

#### 6.4.2 关键词过滤（可选增强）

节点1提取的 `keywords` 可以用于过滤或增强检索：
...
if keywords:
    ...
    results = vector_db_search(enhanced_query)
```
...
节点1：问题加工
  ├─ 输入：用户最新发言 + 历史聊天上下文
  ├─ 输出：query_text（优化后的问题）、keywords（关键词列表）
  └─ 更新状态：query_text, keywords
  ↓
节点2：RAG检索（定制化实现）
  ├─ 输入：query_text, keywords
  ├─ 向量库检索 → retrieved_examples
  ├─ 用户信息查询 → user_info
  └─ 更新状态：retrieved_examples, user_info
  ↓
节点3：核心Agent（逻辑二次判定 + 业务流程处理）
  ├─ 输入：query_text, keywords, retrieved_examples, user_info, 历史对话
  ├─ 逻辑二次判定：意图识别、业务逻辑验证、安全边界判断
  ├─ 更新状态：current_intent
...
class RouterState(TypedDict):
    ...
    # 节点3输出（逻辑二次判定结果）
    current_intent: Optional[str]  # record, query, qa, greeting（节点3判定后设置）
...
1. **问题加工质量**：节点1的查询文本优化效果和关键词提取准确性
2. **检索相关性**：节点2检索的示例是否相关
3. **逻辑判定准确性**：节点3的意图识别和业务逻辑判定准确性
4. **回答质量**：节点3的回答质量是否提升
5. **工具调用成功率**：节点3的工具调用成功率
6. **性能**：整体流程的响应时间

### 8.3 监控指标

1. **节点1**：查询文本优化质量、关键词提取准确性
2. **节点2**：检索命中率、相似度分数分布
3. **节点3**：意图识别准确率、逻辑判定准确性、回答质量、工具调用成功率

---

## 九、总结

本方案通过**问题加工 + RAG增强 + 统一处理**的设计，实现了：

1. **职责清晰**：节点1专注问题加工，节点2专注信息检索，节点3专注业务处理
2. **提示词精简**：节点3只保留核心流程，问答示例通过RAG动态获取
3. **知识库可扩展**：优秀的历史问答和科普资料可随时更新
4. **个性化增强**：结合用户基础信息，提供更精准的回答
5. **流程简化**：从多Agent分离改为三节点流程，更易维护
6. **灵活实现**：节点2采用定制化实现，便于快速迭代和优化

**关键设计点**：
- **节点1（问题加工）**：结合历史上下文，提取问题核心，优化查询文本，提取关键词
- **节点2（RAG检索）**：基于优化后的查询文本和关键词，进行向量库检索和用户信息查询（定制化实现）
- **节点3（核心Agent）**：根据RAG检索信息进行逻辑二次判定，统一处理所有业务流程，执行工具调用
- **向量库**：多表设计，按内容类型分类，支持高效检索

**核心改进**：
- 节点1不再进行意图识别，专注于问题加工和查询文本优化
- 节点2配置简化，采用定制化实现，便于快速迭代
- 节点3承担意图识别和路由职责，进行逻辑二次判定，确保判断准确性

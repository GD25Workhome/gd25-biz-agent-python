...
def create_langfuse_handler(trace_id: Optional[str] = None) -> CallbackHandler:
    """
...
def compile_graph():
    ...
    Returns:
        tuple: (graph, langfuse_handler) - 编译后的图和 Langfuse 处理器
    """
    logger.info("=" * 80)
    logger.info("图编译阶段")
    logger.info("=" * 80)
...
def execute_graph(
    ...
    Args:
        graph: 编译后的图
        langfuse_handler: Langfuse 处理器（如果提供了 trace_id，此参数将被忽略）
        question: 要执行的问题
        question_id: 问题标识（用于日志，可选）
        trace_id: 可选的 Trace ID，用于关联到已存在的 Trace。如果提供，会创建新的 handler
    ...
    """
    if question_id:
        logger.info(f"[执行] 问题 {question_id}: {question}")
    else:
        logger.info(f"[执行] 问题: {question}")
    
    handler_to_use = create_langfuse_handler(trace_id=trace_id)
    # # 如果提供了 trace_id，创建新的 handler
    # handler_to_use = langfuse_handler
    # if trace_id:
    #     logger.info(f"[执行] 使用指定的 Trace ID: {trace_id}")
    #     handler_to_use = create_langfuse_handler(trace_id=trace_id)
    
    # 获取 Langfuse 客户端
    langfuse = get_client()
    
    # 如果提供了 trace_id 或需要设置 trace 元数据，使用 start_as_current_observation
    if trace_id or trace_name or user_id or session_id or metadata:
        # 构建 trace_context（如果提供了 trace_id）
        trace_context = None
        if trace_id:
            # 规范化 trace_id（移除连字符并转换为小写）
        ...
        observation_name = trace_name or f"multi-agent-langgraph-{question_id or 'execution'}"
        with langfuse.start_as_current_observation(
            ...
            logger.info("-" * 80)
            logger.info(f"[执行] 问题 {question_id or '执行'} 完成")
    else:
...
def main():
    ...
    # ========== 阶段一：图编译 ==========
    graph, langfuse_handler = compile_graph()
    ...
    # 可以指定自定义的 trace_id，例如：trace_id="trace_001"
    # 如果不指定（trace_id=None），将使用默认的 handler（自动生成 trace_id）
    # execute_graph(graph, langfuse_handler, question1, question_id="1", trace_id=None)
    
    # 测试问题 2: 询问当前时间
    question2 = "What time is it?"
    # 可以指定自定义的 trace_id，例如：trace_id="trace_002"
    # 如果不指定（trace_id=None），将使用默认的 handler（自动生成 trace_id）
    execute_graph(
        graph, 
        langfuse_handler, 
        ...
        question_id="2", 
        trace_id=None,
        trace_name="multi-agent-langgraph-example-time",
        user_id="test_user",
    ...
    # 示例：使用自定义 trace_id 和 trace name
    import secrets
    ...
    execute_graph(
        ...
        trace_name="multi-agent-langgraph-example-research",
        user_id="test_user",
        session_id="test_session",
        metadata={
            "example": "multi_agent_langgraph_integration",
    ...
    # 刷新 Langfuse 事件
    logger.info("[步骤] 刷新 Langfuse 事件...")
    langfuse = Langfuse()
    langfuse.flush()
    logger.info("[步骤] Langfuse 事件已刷新")

...
华院最小部署：仅注册 agent / function，避免拉起 RAG/AutoGen/Embedding。
"""
import logging
...
class NodeCreatorRegistry:
    ...
    def create_node(self, node_def: NodeDefinition, flow_def: FlowDefinition):
        ...
        if not creator:
            raise ValueError(
                f"不支持的节点类型: {node_def.type}，"
                f"已注册的类型: {list(self._creators.keys())}"
            )
    ...
    def get_creator(self, node_type: str) -> Optional[NodeCreator]:
        """
            ...
            Returns:
                NodeCreator: 节点创建器实例；不存在则返回 None
        """

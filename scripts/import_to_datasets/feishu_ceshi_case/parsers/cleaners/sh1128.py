...
class Sh1128Cleaner(BaseSheetCleaner):
    ...
    def clean(self, row: pd.Series, df: pd.DataFrame) -> List[CanonicalItem]:
        ...
        history_session = convert_to_text(row.get(self._HISTORY_SESSION_COL)) if self._HISTORY_SESSION_COL in df.columns else None
        history_response = convert_to_text(row.get(self._HISTORY_RESPONSE_COL)) if self._HISTORY_RESPONSE_COL in df.columns else None

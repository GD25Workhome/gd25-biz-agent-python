...
if __name__ == "__main__":
    ...
    注意：
        - 以上方式均为开发模式（启用 reload）
        - 生产环境请去掉 --reload 并配置多 worker，例如：
...
**特点**：
- 开发环境使用 `--reload`，代码变更自动重启
- 生产环境去掉 `--reload`，通过 `--workers` 启用多进程
- `--reload` 与 `--workers > 1` 不能同时使用

**适用场景**：
- 带 `--reload`：开发环境
- 无 `--reload` + `--workers`：生产环境（部署前需评估本项目内存队列等多进程影响）

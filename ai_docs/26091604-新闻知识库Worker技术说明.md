...
> **V2 切分（2026-09-16）**：全文固定滑窗 `1000/200/MAX=50` → 多 chunk 写入 `radar_company_news_chunk`；状态机写前先置 `embed_status=0`。详见 `ai_docs/26091607-新闻知识库全文切分与召回策略C设计.md`。
...
| 数据库 | exhibition **MySQL**（三张表） | 本服务 **PostgreSQL** |
...
- Java：**只建任务**，不抓正文、不写向量  
- 本脚本：抢锁后整条写入链跑完  
- gd25 仓储 **只碰三张表**（task / document / agent_guard；不读 url/source 主表；字段已快照在 task 上）
  - Agent 闸门设计见 `ai_docs/26091605-Agent兜底闸门改造设计.md`
...
scripts/news_content_worker.py
  ...
  └─ NewsContentProcessor
       ...
       ├─ agent_fallback           Claude Agent 兜底（分类熔断 / 按源跳过 / 落库配额）
       ├─ agent_guard_store        radar_news_agent_guard

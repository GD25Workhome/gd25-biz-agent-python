"""
update_rewritten_data_node 节点实现。

职责：
- 从 FlowState 中读取 rewritten_agent_node 节点输出的 JSON 解析结果；
- 根据 state 中的 rewritten_record_id（改写任务主键）查记录并更新；
- id 缺失或记录不存在时抛出异常；支持「拿不到数据」场景，更新 status=failed。

设计文档：cursor_docs/021001-Rewritten流程批量异步执行技术设计.md
链路与设计问题：cursor_docs/021002-Rewritten流程update_rewritten_data链路与设计问题.md
"""
...
def _extract_rewritten_record_id(state: FlowState) -> Optional[str]:
    """
    从 state 中提取 rewritten_record_id（改写任务主键）。
    ...
    return _normalize_str(merged.get("rewritten_record_id"))
...
def _build_update_kwargs(
    ...
    """
    构建 DataItemsRewrittenRepository.update 的参数字典。
    不包含 rewritten_record_id（用于定位，不参与 update 字段）。
    """
...
class UpdateRewrittenDataNode(BaseFunctionNode):
    """
    update_rewritten_data_node 节点。

    本节点在 rewritten_agent_node 之后执行，负责将 Agent 输出更新到已有记录。
    前提：记录已由批次创建阶段预写入（status=init 或 processing）。
    """
    ...
    def get_key(cls) -> str:
        ...
        注意：必须与 flow.yaml 中 function_key 保持一致：
            config/flows/pipeline_step2/flow.yaml: function_key: "update_rewritten_data_func"
        """
        return "update_rewritten_data_func"

    async def execute(self, state: FlowState) -> FlowState:
        """
        执行 update_rewritten_data_node 节点逻辑。

        步骤：
        1. 从 state 提取 trace_id、rewritten_record_id；
        2. 从 state.edges_var 提取改写结果；
        3. 根据 rewritten_record_id 查找已有记录，缺失或查不到则抛出异常；
        4. 若有有效数据，更新 status=success 及业务字段；否则更新 status=failed；
        5. 将更新结果摘要写入 state。
        """
        ...
        rewritten_record_id = _extract_rewritten_record_id(state)
        if not rewritten_record_id:
            raise ValueError(
                "update_rewritten_data_node: rewritten_record_id 缺失"
            )
        ...
        async with session_factory() as session:
            ...
            existing = await repo.get_by_id(rewritten_record_id)
            if not existing:
                raise ValueError(
                    f"update_rewritten_data_node: 未找到记录 id={rewritten_record_id!r}"
                )
            await repo.update(rewritten_record_id, **kwargs)
            await session.commit()
            result_summary = {
                "record_id": rewritten_record_id,
                "status": status,
                "updated": 1,
            }
            logger.info(
                "update_rewritten_data_node 执行完成: record_id=%s, status=%s",
                rewritten_record_id,
                status,
            )
        ...
        if "edges_var" not in new_state:
            new_state["edges_var"] = {}
        new_state["edges_var"]["update_rewritten_data_result"] = result_summary

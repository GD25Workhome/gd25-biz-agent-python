...
def execute_graph(graph, langfuse_handler, question: str, question_id: str = None):
    """
    图执行阶段：执行单个问题
    
    Args:
        graph: 编译后的图
        langfuse_handler: Langfuse 处理器
        question: 要执行的问题
...
def main():
    ...
    execute_graph(graph, langfuse_handler, question1, question_id="1")
    
    # 测试问题 2: 询问当前时间
    question2 = "What time is it?"
    execute_graph(graph, langfuse_handler, question2, question_id="2")

...
用 contextvars 在单次 /radar-event-score 请求内向 AnySearch 工具传递
公司主体词、搜索/抽取次数上限等。不依赖医疗登录 Session / Token。
"""
...
class HuayuanRadarEventContextData:
    ...
    def can_search(self, query: str) -> tuple[bool, str]:
        ...
        if q in self.searched_queries:
            return False, f"query 已搜索过，请复用已有结果，勿重复请求: {q[:80]}"
        if self.search_count >= self.max_search_times:
            return False, f"已达搜索上限 max_search_times={self.max_search_times}"
        return True, ""

    def mark_searched(self, query: str) -> None:
        """记录一次已发起的搜索（计入次数与去重集合）。"""
        q = str(query or "").strip()
...
def build_radar_event_context_from_request(
    *,
    ...
    return HuayuanRadarEventContextData(
        ...
        max_search_times=max(0, int(max_search_times)),

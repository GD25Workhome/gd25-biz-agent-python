...
(function() {
    ...
    const { defineComponent, ref, reactive, computed, watch } = Vue;
    ...
    window.PipelineDatasetItemsComponent = defineComponent({
        ...
        setup(props) {
            ...
            return {
                ...
                loadItems, openItemCreate, openItemEdit, submitItemForm, deleteItem, toggleItemStatus,
                onSearch, onResetQuery, onPageChange, onSizeChange,
                Plus: icons.Plus, Edit: icons.Edit, Delete: icons.Delete, Search: icons.Search, ArrowDown: icons.ArrowDown, ArrowUp: icons.ArrowUp
            };
        },
        template: `
        <div style="height:100%;width:100%;min-width:0;display:flex;flex-direction:column;background:#fff;">
            <div style="padding:12px 20px;border-bottom:1px solid #e4e7ed;display:flex;justify-content:space-between;align-items:center;">
                <span style="font-weight:600;">{{ datasetName || '数据项' }} — 共 {{ itemsTotal }} 条</span>
                    ...
                    <el-table-column type="expand" width="48">
                        <template #default="props">
                            <div style="padding:12px 24px;background:#fafafa;">
                                <p v-if="props.row.input"><strong>input：</strong><pre style="margin:4px 0;font-size:12px;max-height:120px;overflow:auto;">{{ JSON.stringify(props.row.input,null,2) }}</pre></p>
                                <p v-if="props.row.output"><strong>output：</strong><pre style="margin:4px 0;font-size:12px;max-height:120px;overflow:auto;">{{ JSON.stringify(props.row.output,null,2) }}</pre></p>
            ...
            <el-dialog v-model="itemDialogVisible" :title="itemDialogMode==='create'?'新建数据项':'编辑数据项'" width="520px">
                <el-form :model="itemForm" label-width="100px">
                    <el-form-item label="unique_key"><el-input v-model="itemForm.unique_key" /></el-form-item>
                    <el-form-item label="input"><el-input v-model="itemForm.input" type="textarea" placeholder="JSON" :rows="3" /></el-form-item>
                    <el-form-item label="output"><el-input v-model="itemForm.output" type="textarea" placeholder="JSON" :rows="3" /></el-form-item>
                    <el-form-item label="metadata"><el-input v-model="itemForm.metadata" type="textarea" placeholder="JSON" :rows="2" /></el-form-item>

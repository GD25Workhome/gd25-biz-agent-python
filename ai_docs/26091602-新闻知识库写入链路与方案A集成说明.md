...
## 2. 表结构与状态机（gd25 只碰三张表）

仓储硬边界：`backend/domain/news_content/repository.py` / `agent_guard_store.py` **读写**下面三张表；不读 `radar_company_news_url` / `radar_company_source_url`（建任务时所需字段已快照进 task）。

- `radar_news_content_task`
- `radar_company_news_document`
- `radar_news_agent_guard`（Agent 兜底闸门按日计数，见 `ai_docs/26091605`）
...
| `AgentFallbackGuard`     | 同上 + `agent_guard_store`         | 开关 + 日配额原子扣减 + system 熔断 + 按 source_url_id 内容跳过（**落库**） |
